...
export const POST = secureEndpoint(
  async (req: NextRequest) => {
    try {
      ...
      const settlement = await SavingsBillingEngine.processManualSingleBilling(targetAccountId);
      ...
      return NextResponse.json(
        { error: error instanceof Error ? error.message : "Failed to execute manual fee settlement." },
        { status: 500 }
      );
    }
  },
...
export async function GET() {
  if (process.env.NODE_ENV === "production" && !process.env.ALLOW_IPN_REGISTRATION_PROD) {
    return NextResponse.json({ error: "Not authorized in this environment context." }, { status: 403 });
  }

  const BASE_URL = process.env.PESAPAL_BASE_URL; 
  const CONSUMER_KEY = process.env.PESAPAL_CONSUMER_KEY;
  const CONSUMER_SECRET = process.env.PESAPAL_CONSUMER_SECRET;
  const MY_DOMAIN = process.env.NEXT_PUBLIC_APP_URL || "https://ukuafin.com";
  ...
  try {
    ...
    if (!token) {
      return NextResponse.json({ error: "Failed to grab bearer token from Pesapal.", details: authData }, { status: 400 });
    }
    ...
    const errorMessage = error instanceof Error ? error.message : "Failed to execute registration routing task.";
